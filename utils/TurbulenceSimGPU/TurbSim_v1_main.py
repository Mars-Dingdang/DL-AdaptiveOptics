...
def _use_cuda(device: object) -> bool:
...
def _fftconvolve2d_torch_same_tensors(a_t: "torch.Tensor", b_t: "torch.Tensor") -> "torch.Tensor":
    """Torch FFT version of scipy.signal.fftconvolve(a, b, mode='same')."""
    out_h = int(a_t.shape[0] + b_t.shape[0] - 1)
    out_w = int(a_t.shape[1] + b_t.shape[1] - 1)
    fa = torch.fft.rfft2(a_t, s=(out_h, out_w))
    fb = torch.fft.rfft2(b_t, s=(out_h, out_w))
    full = torch.fft.irfft2(fa * fb, s=(out_h, out_w))
    start_h = (out_h - int(a_t.shape[0])) // 2
    start_w = (out_w - int(a_t.shape[1])) // 2
    return full[start_h : start_h + int(a_t.shape[0]), start_w : start_w + int(a_t.shape[1])]


def _fftconvolve2d_torch_same(a: np.ndarray, b: np.ndarray, device: str) -> np.ndarray:
    """Compatibility wrapper returning numpy output."""
    a_t = torch.as_tensor(a, dtype=torch.float32, device=device)
    b_t = torch.as_tensor(b, dtype=torch.float32, device=device)
    same = _fftconvolve2d_torch_same_tensors(a_t, b_t)
    return same.detach().cpu().numpy()
...
def genTiltImg(img, p_obj):
    ...
    if (p_obj.get('S') == None).any():
        ...
        p_obj['S'] = S
        flag_noPSD = 1
    device = p_obj.get("device")
    scale = np.sqrt(2) * 2 * p_obj['N'] * (p_obj['L'] / p_obj['delta0'])
    if _use_cuda(device):
        S_t = torch.as_tensor(p_obj['S'], device=device)
        rand_x = torch.as_tensor(np.random.randn(2 * p_obj['N'], 2 * p_obj['N']), dtype=torch.float32, device=device)
        rand_y = torch.as_tensor(np.random.randn(2 * p_obj['N'], 2 * p_obj['N']), dtype=torch.float32, device=device)
        MVx_t = torch.fft.ifft2(S_t * rand_x).real * scale
        MVy_t = torch.fft.ifft2(S_t * rand_y).real * scale
        MVx = MVx_t[round(p_obj['N'] / 2) : 2 * p_obj['N'] - round(p_obj['N'] / 2), 0 : p_obj['N']].detach().cpu().numpy()
        MVy = MVy_t[0 : p_obj['N'], round(p_obj['N'] / 2) : 2 * p_obj['N'] - round(p_obj['N'] / 2)].detach().cpu().numpy()
    else:
        MVx = np.real(np.fft.ifft2(p_obj['S'] * np.random.randn(2 * p_obj['N'], 2 * p_obj['N']))) * scale
        ...
        MVy = np.real(np.fft.ifft2(p_obj['S'] * np.random.randn(2 * p_obj['N'], 2 * p_obj['N']))) * scale
    ...
    img_ = motion_compensate(img, MVx - np.mean(MVx), MVy - np.mean(MVy), 0.5, device=device)
...
def genBlurImage(p_obj, img):
    ...
    patch_grid_downsample = max(1, int(p_obj.get("patch_grid_downsample", 1)))
    psf_resolution = max(8, int(p_obj.get("psf_resolution", 32)))
    reuse_psf_per_frame = bool(p_obj.get("reuse_psf_per_frame", False))
    smax = p_obj['delta0'] / p_obj['D'] * p_obj['N']
    temp = np.arange(1,101)
    patchN = temp[np.argmin((smax*np.ones(100)/temp - 2)**2)]
    patchN = max(1, int(round(float(patchN) / float(patch_grid_downsample))))
    patch_size = round(p_obj['N'] / patchN)
    xtemp = np.round(p_obj['N']/(2*patchN) + np.linspace(0, p_obj['N'] - p_obj['N']/patchN + 0.001, patchN))
    xx, yy = np.meshgrid(xtemp, xtemp)
    xx_flat, yy_flat = xx.flatten(), yy.flatten()
    NN = int(psf_resolution) # For extreme scenarios, increase this for fidelity.
    num = np.zeros((p_obj['N'], p_obj['N']))
    ...
    patch_envelope = np.exp(-patch_indx**2/patch_size**2)*np.exp(-patch_indy**2/patch_size**2)*np.ones((patch_size*2+1, patch_size*2+1))

    if use_cuda:
        img_t = torch.as_tensor(img, dtype=torch.float32, device=device)
        num_t = torch.zeros((p_obj['N'], p_obj['N']), dtype=torch.float32, device=device)
        den_t = torch.zeros((p_obj['N'], p_obj['N']), dtype=torch.float32, device=device)
        patch_envelope_t = torch.as_tensor(patch_envelope, dtype=torch.float32, device=device)

    cached_psf_np = None
    cached_psf_t = None
    if reuse_psf_per_frame:
        ...
        cached_psf_np = psf
        if use_cuda:
            cached_psf_t = torch.as_tensor(psf, dtype=torch.float32, device=device)
    ...
    for i in range(int(patchN**2)):
        ...
        if use_cuda:
            patch_mask_t = torch.as_tensor(patch_mask, dtype=torch.float32, device=device)
            patch_mask_t = _fftconvolve2d_torch_same_tensors(patch_mask_t, patch_envelope_t)
            psf_t = cached_psf_t if reuse_psf_per_frame and cached_psf_t is not None else torch.as_tensor(psf, dtype=torch.float32, device=device)
            den_t += _fftconvolve2d_torch_same_tensors(patch_mask_t, psf_t)
            num_t += _fftconvolve2d_torch_same_tensors(img_t * patch_mask_t, psf_t)
        else:
            patch_mask = scipy.signal.fftconvolve(patch_mask, patch_envelope, mode='same')
            den += scipy.signal.fftconvolve(patch_mask, psf, mode='same')
            num += scipy.signal.fftconvolve(img * patch_mask, psf, mode='same')

    if use_cuda:
        out_img = (num_t / (den_t + 0.000001)).detach().cpu().numpy()
    else:
        out_img = num / (den + 0.000001)
    return out_img
...
def genZernikeCoeff(num_zern, D_r0):
    ...
    :return:
    '''
    R = _cached_zernike_factor(int(num_zern))
...
@lru_cache(maxsize=8)
def _cached_zernike_factor(num_zern: int) -> np.ndarray:
    """Cache eigendecomposition term used in Zernike coefficient sampling."""
    C = nollCovMat(num_zern, 1, 1)
    e_val, e_vec = np.linalg.eig(C)
    e_val = np.clip(np.real(e_val), 0.0, None)
    return np.real(e_vec * np.sqrt(e_val))
...
def radialZernike(x_grid, y_grid, z_ind):
    ...
    for k in range(int((n - m)/2 + 1)):
        #print(k)
        temp = (-1) ** k * math.factorial(int(n - k)) / (
