"""Build a reproducible single-frame evaluation LMDB for sequence-trained models.

This script selects a contiguous slice of clean images, generates one degraded
frame per image, replicates that frame to a fixed-length sequence, writes the
result to LMDB, and exports side-by-side clean/degraded previews.
"""
...
from utils.degradation import add_atmospheric_turbulence_sequence, sample_turbulence_context
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build single-frame eval LMDB and degradation previews.")
    ...
    parser.add_argument(
        "--output-lmdb-root",
        type=Path,
        default=Path("data/eval_single_frame"),
        help="LMDB output directory.",
    )
    parser.add_argument(
        "--viz-dir",
        type=Path,
        default=Path("outputs/eval_single_frame_degradation"),
        help="Directory for exported clean-vs-degraded preview PNGs.",
    )
    ...
    parser.add_argument("--num-frames", type=int, default=7, help="Replicated sequence length stored in LMDB.")
...
def _save_preview(clean_rgb: np.ndarray, degraded_rgb: np.ndarray, out_path: Path) -> None:
    clean_labeled = _put_label(clean_rgb, "Clean")
    degraded_labeled = _put_label(degraded_rgb, "Degraded")
    preview = np.concatenate([clean_labeled, degraded_labeled], axis=1)
...
def main() -> None:
    ...
    try:
        with env.begin(write=True) as txn:
            for sample_idx, src_path in enumerate(selected):
                ...
                degraded_frame = np.clip(degraded_seq[0], 0.0, 1.0).astype(np.float32)
                degraded_uint8 = np.clip(np.round(degraded_frame * 255.0), 0.0, 255.0).astype(np.uint8)
                ...
                for frame_idx in range(int(args.num_frames)):
                    txn.put(
                        ...
                        _encode_rgb_image_bytes(degraded_uint8, codec=codec, quality=int(args.image_quality)),
                    )

                frame_meta = dict(frame_metas[0]) if frame_metas else {}
                meta = {
                    "sample_id": f"{sample_idx:07d}",
                    "source_index": int(args.start_index) + sample_idx,
                    "source": str(src_path.as_posix()),
                    "num_frames": int(args.num_frames),
                    "mode": "single_frame_repeated",
                ...
                _save_preview(clean_rgb=clean_rgb, degraded_rgb=degraded_uint8, out_path=args.viz_dir / preview_name)
    ...
    print(f"[INFO] Built single-frame eval LMDB at: {args.output_lmdb_root}")
    print(f"[INFO] Exported degradation previews to: {args.viz_dir}")
